...
fetchDataFromServer(
  ...
  function ({ genres }) {
    ...
    fetchDataFromServer(
      `https://api.themoviedb.org/3/movie/now_playing?api_key=${api_key}&language=zh-HK&page=1&region=HK`,
      heroBanner
    );
  }
);
/* movies now in theatres */
const heroBanner = function ({ results: movieList }) {
  const banner = document.createElement("section");
  banner.classList.add("banner");
  banner.ariaLabel = "Movies in HK Theatres";
  ...
  for (const [index, movie] of movieList.entries()) {
    ...
    const detailURL = `./detail.html?movieId=${id}`;
    ...
    sliderItem.innerHTML = `
      ...
      <div class="banner-content">
        ...
        <div class="meta-list">
          <div class="meta-items">${release_date}</div>
          ...
          <a href="${detailURL}" class="btn" onClick="getMovieDetail(${id})">
            <img
    ...
    controlItem.addEventListener("wheel", transformScroll);
...
const createMovieList = function ({ results: movieList }, title) {
  ...
  movieListElem.querySelector(".slider-list").addEventListener("wheel", transformScroll);
